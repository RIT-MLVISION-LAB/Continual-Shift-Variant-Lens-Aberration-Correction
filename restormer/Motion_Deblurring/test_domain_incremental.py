...
def adapter_id_to_variant(adapter_id):
    """Inverse: -1→V1, 0→V2, 1→V3, ..."""
    return adapter_id + 2
...
def evaluate(args):
    ...
    for v in sorted(results.keys()):
        ...
        print(f"V{v:<9} {acc:>5.1f}%      {psnr_p:>6.2f} dB     "
    ...
    header = "       " + "".join(f"  V{v:<4}" for v in variant_list)
    print(header)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Domain-incremental evaluation with prototype-based "
        "task inference for RestormerAdapters"
    )
    ...
    parser.add_argument(
        ...
        help="Path to prototypes .pth file",
    )
    parser.add_argument(
        "--data_root", type=str,
